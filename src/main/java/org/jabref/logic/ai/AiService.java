...
public class AiService {
    ...
    private final Map<BibDatabase, BibDatabaseChats> bibDatabaseChatsMap = new HashMap<>();
    ...
    public @Nullable BibDatabaseChats openBibDatabaseChats(BibDatabaseContext bibDatabaseContext) {
        ...
        if (bibDatabaseContext.getDatabasePath().isPresent()) {
            if (bibDatabaseChatsMap.containsKey(bibDatabase)) {
                return bibDatabaseChatsMap.get(bibDatabase);
            }

            // TODO: Error handling??????
            BibDatabaseChats bibDatabaseChats = new BibDatabaseChats(bibDatabaseContext.getDatabasePath().get().toString());

            bibDatabaseChatsMap.put(bibDatabase, bibDatabaseChats);
